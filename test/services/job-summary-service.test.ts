...
describe("Job Summary Service", () => {
  ...
  describe("createJobSummary", () => {
    it("should create job summary with results", async () => {
      ...
      expect(mockSummary.addRaw).toHaveBeenCalledWith(expect.stringContaining("test.md"));
      expect(mockSummary.addRaw).toHaveBeenCalledWith(expect.stringContaining("example.md"));
      expect(mockSummary.addRaw).toHaveBeenCalledWith(expect.stringContaining("## 📊 Summary"));
      expect(mockSummary.write).toHaveBeenCalled();
      expect(infoSpy).toHaveBeenCalledWith("✅ Job summary created successfully");
    });

    it("should handle empty results", async () => {
      await jobSummaryService.createJobSummary([], mockConfig, "schedule", mockContext);
    ...
    it("should include configuration details", async () => {
      await jobSummaryService.createJobSummary(mockResults, mockConfig, "schedule", mockContext);
    ...
    it("should include quality score legend", async () => {
      ...
      expect(mockSummary.addRaw).toHaveBeenCalledWith(
        expect.stringContaining("*Quality Score Legend: 🟢 80+ | 🟡 60-79 | 🔴 0-59*"),
      );
    });
  });
});
