  ...
  describe("Validation Guards", () => {
    describe("isValidSHA", () => {
      it("should return true for valid SHA formats", () => {
        ...
        expect(isValidSHA("a1b2c3d4e5f6789012345678901234567890abcd")).toBe(true); // Full SHA-1
        expect(isValidSHA("ABCDEF1234567890abcdef1234567890abcdef12")).toBe(true); // Uppercase
      });

      it("should return false for invalid SHA formats", () => {
        expect(isValidSHA("abc123")).toBe(false); // Too short
        expect(isValidSHA("abc12345678901234567890123456789012345678901")).toBe(false); // Too long
    ...
    describe("isGitHubAPIError", () => {
      it("should return true for GitHub API errors", () => {
        expect(isGitHubAPIError({ status: 404, message: "Not found" })).toBe(true);
  ...
  describe("Safe Access Utilities", () => {
    describe("safeGet", () => {
      ...
      it("should return undefined for null/undefined objects", () => {
        expect(safeGet<Record<string, unknown>, string>(null, "key")).toBeUndefined();
        expect(safeGet<Record<string, unknown>, string>(undefined, "key")).toBeUndefined();
      });
    });
    ...
    describe("safeArrayGet", () => {
      ...
      it("should return undefined for invalid indices", () => {
        ...
        expect(safeArrayGet(arr, createInvalidNumber(undefined))).toBeUndefined();
      });
    });
  });
});
